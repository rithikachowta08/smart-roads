...
car_cascade = CascadeClassifier('oldcar.xml')  # XML file describing features of cars
truck_cascade = CascadeClassifier('truck.xml')   #XML file decribing some features of trucks 
bike_cascade = CascadeClassifier('bike11.xml')  #XML file decribing some features of bikes
...
while True:
    ...
    if cars1 is not () or trucks1 is not () or bike1 is not () :
	 ...
	 for i in range(len(trucks1)):  
		if weights3[i][0] > 3.0:		 # To draw a rectangle on each car from the current frame
    ...
    if cars2 is not () or trucks2 is not () or bike2 is not ():
	 ...
	 for i in range(len(trucks2)):  
		if weights4[i][0] > 3.0:		 # To draw a rectangle on each car from the current frame
